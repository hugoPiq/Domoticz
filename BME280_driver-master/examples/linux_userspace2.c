...
void export_pin(char *number);
void unexport_pin(char *number);
void write_direction_red_pin(char *direction);
void write_direction_green_pin(char *direction);
void write_value_red_pin(char *value);
void write_value_green_pin(char *value);

void signals_handler(int signal_number)
{
    printf("Signal catched.\n");
    //Switch off the leds
    write_value_green_pin("0");
    write_value_red_pin("0");
...
void write_direction_green_pin(char *direction)
{
    int fd = open("/sys/class/gpio/gpio5/direction", O_WRONLY);
    if (fd == -1)
    {
...
void write_value_red_pin(char *value)
{
    int fd_red = open("/sys/class/gpio/gpio6/value", O_WRONLY);
    if (fd_red == -1)
    {
        perror("Unable to open /sys/class/gpio/gpio6/value");
        exit(1);
    }
    if (write(fd_red, value, 1) != 1)
    {
...
void write_value_green_pin(char *value)
{
    int fd_green = open("/sys/class/gpio/gpio5/value", O_WRONLY);
    if (fd_green == -1)
    {
        perror("Unable to open /sys/class/gpio/gpio5/value");
        exit(1);
    }
    if (write(fd_green, value, 1) != 1)
    {
...
int8_t stream_sensor_data_forced_mode(struct bme280_dev *dev)
{
    ...
    write_direction_red_pin("out");
    write_direction_green_pin("out");
    ...
    while (1)
    {
        ...
        if (comp_data.humidity > 80)
        {
            write_value_red_pin("1");
            write_value_green_pin("0");
        }
        else
        {
            write_value_red_pin("0");
            write_value_green_pin("1");
        }
        usleep(50000);
    }
